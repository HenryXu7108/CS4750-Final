...
if (isset($_POST['income_id'], $_POST['amount'], $_POST['date'], $_POST['category'], $_POST['source'])) {
    ...
    if (strlen($source) > 255) {
        die("source must be 255 characters or less.");
    }


    // Update the database
    $stmt = $db->prepare("UPDATE Income SET amount = :amount, date = :date, category = :category , source = :source WHERE income_id = :income_id");
